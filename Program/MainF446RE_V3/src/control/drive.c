...
static void UpdateWheelSpeeds(Drive *obj) {
  ...
  obj->rear_speed_left_m_s = omega_left / DRIVE_GEAR_RATIO * DRIVE_REAR_WHEEL_RADIUS_M;
  obj->rear_speed_right_m_s = omega_right / DRIVE_GEAR_RATIO * DRIVE_REAR_WHEEL_RADIUS_M;
}
...
void Drive_Update(Drive *obj) {
  ...
  if (Abs(obj->target_speed_m_s) < DRIVE_STANDSTILL_SPEED_M_S &&
  ...
  float left_nm = requested_total_nm * 0.5f - obj->yaw_moment_torque_nm * 0.5f;
  float right_nm = requested_total_nm * 0.5f + obj->yaw_moment_torque_nm * 0.5f;
  ...
  // 最高速リミッタ: 車速フィードバックが壊れて積分が振り切れても、実速度が上限を超えたら加速させない
  if (obj->vehicle_speed_m_s > DRIVE_MAX_SPEED_M_S) {
    if (left_nm > 0.0f) left_nm = 0.0f;
    if (right_nm > 0.0f) right_nm = 0.0f;
  } else if (obj->vehicle_speed_m_s < -DRIVE_MAX_SPEED_M_S) {
    if (left_nm < 0.0f) left_nm = 0.0f;
    if (right_nm < 0.0f) right_nm = 0.0f;
  }
  ...
  if (dt_s > 0.0f && obj->speed_pid.ki > 0.0f) {
    float applied_total_nm = left_nm + right_nm;
    obj->speed_pid.integral +=
        (applied_total_nm - requested_total_nm) / obj->speed_pid.ki * (dt_s / DRIVE_ANTIWINDUP_TT_S);
  }
