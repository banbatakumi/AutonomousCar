...
## 2026-08-09: プロトコル v0.5 — 灯火/パッシング/クラクション/ブレーキ強さの操作を追加

Pi 側 GUI からクラクション・パッシング・灯火切替 (OFF/DAYTIME/NORMAL) を操作できるようにし、
ブレーキを on/off ではなく N・m で指定できるようにした。詳細と Pi 側の対応チェックリストは
[pi_uart_protocol_v0.5_delta.md](pi_uart_protocol_v0.5_delta.md)。

### 変更点

- **`COMMAND` (0x10) の LEN 10 → 12**、`protocol_version` 0x0004 → 0x0005。
  末尾に `brake_torque : u16` (0.0001 N·m/LSB、0 = 未指定 → 最大制動) を追加。
  `flags` は bit3 の1ビット `light` を bit3-4 の2ビット `light_mode` に拡張し、bit5 に
  `passing` を新設。**Pi 側を更新するまで走行指令は LEN 不一致で全て破棄される** (意図的)。
- **ブレーキが車速PIを迂回するようになった** (`Drive_SetBrake`)。従来は目標車速0を与えるだけで
  制動力がPIゲイン任せだったため、N・m で指定した強さどおりに効かなかった。
- **クラクションが「押している間だけ鳴る」に変更** (連続トーン。従来は立ち上がりで300msの単発)。
- **緊急停止・COMMAND 途絶時の処置を `ApplyFailsafe()` に統合**し、最大制動トルクで止めるように
  した (従来は目標車速0のみ)。併せてクラクションとパッシングを強制解除する。
- **`torque_cmd` の符号規約を明文化**: 正=駆動、負=制動。停車保持中も制動トルクを負値で報告する
  ようになった (従来は0で、押さえているのか無出力なのか区別できなかった)。

### 起動時に駆動電源が一瞬 ON になる問題を修正

`Setup()` が毎回ステアリング原点処理の前後で `DRIVE_POWER` を ON→OFF していた。MD との通信が
要るのは**ボタン1を押しながら起動して較正するときだけ**で、通常起動は Flash から読むだけなので
給電は不要。`if (calibrate_steering)` で囲い、較正しない起動では一度も投入しないようにした。
