...
void RemoteManual_Run(const RemoteCommand* cmd, const LD06* lidar) {
  ...
  if (cmd->enable_auto_brake) {
    bool front_blocked = Lidar_GetSector(lidar, 0, 20).avg < AUTO_BRAKE_DISTANCE_MM + Drive_GetSpeed() * 150.0f && cmd->move_speed > 0;
    bool rear_blocked = Lidar_GetSector(lidar, 180, 20).avg < AUTO_BRAKE_DISTANCE_MM - Drive_GetSpeed() * 150.0f && cmd->move_speed < 0;
